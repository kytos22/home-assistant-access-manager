...
Access Manager never asks for, receives, embeds, logs, or stores Wi-Fi, API-encryption, or OTA secret values. It also does not request broad access to ESPHome's configuration directory. This keeps the add-on isolated from device credentials and prevents an administration-panel compromise from automatically becoming a firmware-credential compromise.
...
Directly writing files into `/config/esphome` from Access Manager was rejected because it would require broader filesystem permissions, couple two add-ons, and increase the impact of a panel vulnerability. A future guided installer may be considered for the fixed reference display after secure provisioning, backup, recovery, and credential rotation are designed and tested.
