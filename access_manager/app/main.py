...
def validate_esphome_base_url(value):
    ...
    if (
        ...
        raise ValueError("Enter a valid HTTP or HTTPS Device Builder URL")
    return value


def validate_esphome_configuration(value):
    value = str(value or "").strip()
    if (
        not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,126}\.ya?ml", value)
        or value.lower() == "secrets.yaml"
    ):
...
def esphome_secret_keys(payload):
    keys = {}
    for field, default in (
        ...
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,63}", value):
            raise ValueError(
                f"Invalid ESPHome secret key name for {field.replace('_', ' ')}"
            )
        keys[field] = value
    return keys
...
def esphome_reader_config(payload):
    ...
    if reader_id:
        lines.append(f"# Access Manager reader ID: {reader_id}")
    if configuration:
        lines.append(f"# Canonical Device Builder configuration: {configuration}")
    ...
    lines.extend(
        [
            f"  wifi_ssid_value: !secret {secret_keys['wifi_ssid_secret']}",
            f"  wifi_password_value: !secret {secret_keys['wifi_password_secret']}",
            f"  api_encryption_key_value: !secret {secret_keys['api_encryption_key_secret']}",
            f"  ota_password_value: !secret {secret_keys['ota_password_secret']}",
        ]
    )
    lines.extend(
        [
            ...
            f"    ref: {READER_FIRMWARE_REF}",
            "    files:",
            f"      - {READER_FIRMWARE_DIRECTORY}/{package_file}",
            "    refresh: 1d",
            "",
...
class Registry:
    def __init__(self, path):
        ...
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS firmware_operations (
                id TEXT PRIMARY KEY,
                reader_id TEXT,
                configuration TEXT NOT NULL,
                install INTEGER NOT NULL DEFAULT 0,
                remote_job_id TEXT,
                remote_tail_job_id TEXT,
    ...
    def esphome_secret_keys(self):
        try:
            saved = json.loads(self.setting("esphome_secret_keys") or "{}")
        except (TypeError, json.JSONDecodeError):
            saved = {}
        return esphome_secret_keys(saved if isinstance(saved, dict) else {})

    def set_esphome_secret_keys(self, payload):
        keys = esphome_secret_keys(payload)
        self.set_setting("esphome_secret_keys", json.dumps(keys))
        return keys
    ...
    def esphome_cipher(self):
        if self._esphome_cipher is not None:
            return self._esphome_cipher
        key_path = DATA_DIR / "esphome_connection.key"
        try:
            key = key_path.read_bytes().strip()
        except FileNotFoundError:
            DATA_DIR.mkdir(parents=True, exist_ok=True)
        ...
        try:
            os.chmod(key_path, 0o600)
            self._esphome_cipher = Fernet(key)
        except (OSError, TypeError, ValueError) as error:
            raise RuntimeError("Invalid Device Builder encryption key") from error
        return self._esphome_cipher

    def esphome_connection(self, include_token=False):
        try:
            connection = json.loads(self.setting("esphome_connection") or "{}")
        except (TypeError, json.JSONDecodeError):
            connection = {}
        ...
        if ciphertext:
            try:
                token = self.esphome_cipher().decrypt(
    ...
    def save_esphome_connection(
        ...
        connection = {
            ...
            "last_error": None,
            "server_version": str(server_info.get("server_version", ""))[:80],
            "esphome_version": str(server_info.get("esphome_version", ""))[:80],
            "ha_addon": bool(server_info.get("ha_addon")),
            "ha_ingress": bool(server_info.get("ha_ingress")),
            "requires_auth": bool(server_info.get("requires_auth")),
            "device_count": max(0, int(device_count)),
            "api_compatible": True,
            "version_history_enabled": bool(
                (preferences or {}).get("version_history_enabled", True)
            ),
        }
        token = str(token or "").strip()
        ciphertext = (
            self.esphome_cipher().encrypt(token.encode("utf-8")).decode("ascii")
            if token else None
        )
        with self.connection:
            self.connection.execute(
            ...
            if ciphertext:
                self.connection.execute(
                    """
                    INSERT INTO settings(key, value) VALUES ('esphome_token', ?)
                    ON CONFLICT(key) DO UPDATE SET value = excluded.value
                    """,
                    (ciphertext,),
                )
                ...
                self.connection.execute(
                    "DELETE FROM settings WHERE key = 'esphome_token'"
                )
    ...
    def mark_esphome_connection_error(self, error):
        ...
        connection.pop("configured", None)
        connection.pop("token_configured", None)
        self.set_setting("esphome_connection", json.dumps(connection))
    ...
    def public_firmware_operation(row):
        operation = dict(row)
        try:
            operation["logs"] = json.loads(operation.pop("logs_json") or "[]")
        except (TypeError, json.JSONDecodeError):
            operation["logs"] = []
        operation["install"] = bool(operation["install"])
        return operation
    ...
    def create_firmware_operation(
        ...
        self.connection.commit()
        return self.firmware_operation(operation_id)
    ...
    def update_firmware_operation(self, operation_id, **changes):
        ...
        for key, value in changes.items():
            ...
            values.append(json.dumps(value[-80:]) if key == "logs" else value)
        if not assignments:
            return self.firmware_operation(operation_id)
        assignments.append("updated_at = ?")
        values.extend((now_iso(), operation_id))
        self.connection.execute(
            f"UPDATE firmware_operations SET {', '.join(assignments)} WHERE id = ?",
            values,
        )
        self.connection.commit()
        return self.firmware_operation(operation_id)

    def firmware_operation(self, operation_id):
        row = self.connection.execute(
            "SELECT * FROM firmware_operations WHERE id = ?", (operation_id,)
        ).fetchone()
        return self.public_firmware_operation(row) if row else None
    ...
    def firmware_operations(self, pending_only=False, limit=50):
        ...
        rows = self.connection.execute(
            f"SELECT * FROM firmware_operations {where} "
            "ORDER BY created_at DESC LIMIT ?",
            (limit,),
        ).fetchall()
        return [self.public_firmware_operation(row) for row in rows]
...
class FingerprintAdmin:
    ...
    async def startup(self, _app):
        ...
        if self.registry.esphome_connection().get("configured"):
            task = asyncio.create_task(self.refresh_device_builder_cache())
            self.firmware_tasks.add(task)
            task.add_done_callback(self.firmware_tasks.discard)
        for operation in self.registry.firmware_operations(pending_only=True):
            task = asyncio.create_task(
                self.follow_device_builder_operation(operation["id"])
            )
            self.firmware_tasks.add(task)
            task.add_done_callback(self.firmware_tasks.discard)
    ...
    @staticmethod
    def public_esphome_devices(result):
        ...
        for item in devices:
            if not isinstance(item, dict):
                continue
            configuration = str(
                item.get("configuration") or item.get("filename") or ""
            ).strip()
            name = str(item.get("name") or "").strip()
            if not configuration and name:
            ...
            public.append(
                {
                    ...
                    "name": name,
                    "friendly_name": str(
                        item.get("friendly_name") or item.get("comment") or ""
                    ).strip()[:80],
                    "address": str(item.get("address") or "").strip()[:120],
                    "state": str(item.get("state") or item.get("status") or "").strip()[:40],
                }
            )
        return sorted(public, key=lambda item: item["configuration"].lower())
    ...
    async def device_builder_devices(self, client=None):
        ...
        devices = self.public_esphome_devices(
            await client.command("devices/list")
        )
        self.esphome_devices_cache = devices
        return devices

    async def refresh_device_builder_cache(self):
        try:
    ...
    def device_builder_response(error):
        return web.json_response(
            ...
            status=502 if error.code != "not_configured" else 409,
        )
        ...
        return web.json_response(
            {
                ...
                "server_info": {
                    ...
                    for key in (
                        "server_version", "esphome_version", "ha_addon",
                        "ha_ingress", "requires_auth",
                    )
                },
                "devices": devices,
    ...
    async def save_esphome_connection(self, request):
        self.require_admin_request(request)
        payload = await request.json()
        saved = self.registry.esphome_connection(include_token=True)
        try:
            ...
            return self.device_builder_response(error)
        return web.json_response(
            {"ok": True, "connection": connection, "devices": devices}
        )

    async def delete_esphome_connection(self, request):
        self.require_admin_request(request)
    ...
    async def list_esphome_devices(self, request):
        self.require_admin_request(request)
        try:
            devices = await self.device_builder_devices()
        except DeviceBuilderError as error:
            self.registry.mark_esphome_connection_error(error)
            return self.device_builder_response(error)
        return web.json_response({"ok": True, "devices": devices})

    async def esphome_preview_data(self, payload):
        ...
        target = next(
            (item for item in devices if item["configuration"] == configuration),
            None,
        )
        same_name = next(
            (
        ...
        collision = None
        if install_mode == "new" and target:
            collision = "That Device Builder configuration already exists"
        elif install_mode == "existing" and not target:
            collision = "Select an existing Device Builder configuration"
        elif same_name:
            collision = (
                f"Device name {device_name} is already used by "
                f"{same_name['configuration']}"
            )
        elif target and target.get("name") and target["name"] != device_name:
            collision = (
                f"{configuration} belongs to Device Builder device "
                f"{target['name']}"
            )
        version_history_enabled = None
        if target and not collision:
            preferences = await client.command("config/get_preferences")
            if isinstance(preferences, dict):
                history = preferences.get("version_history")
                version_history_enabled = bool(
                    preferences.get("version_history_enabled")
                    or preferences.get("git_enabled")
                    or (isinstance(history, dict) and history.get("enabled"))
                )
            else:
                version_history_enabled = False
            if not version_history_enabled:
                collision = (
                    "Enable Device Builder version history before overwriting "
    ...
    def save_esphome_firmware_profile(self, payload, configuration=None):
        reader_id = str(payload.get("reader_id") or "").strip()
        if not reader_id:
            return None
        reader = self.registry.reader(reader_id)
        ...
        if profile == "display":
            firmware_profile["display_language"] = str(
                payload.get("display_language", "English")
            )
        else:
            firmware_profile["hardware"] = {
                key: str(payload.get(key, "")).strip()
                for key in ("board", "fingerprint_tx_pin", "fingerprint_rx_pin")
            }
        config = dict(reader.get("config") or {})
        config.update(
            esphome_configuration=configuration,
            device_name=firmware_profile["device_name"],
            hardware_profile=profile,
            firmware_secret_keys=firmware_profile["secret_keys"],
            firmware_profile=firmware_profile,
        )
        if profile == "display":
            config["display_language"] = firmware_profile["display_language"]
        else:
            config["firmware_config"] = firmware_profile["hardware"]
        self.registry.update_reader(
            reader_id, reader["name"], reader.get("door_id") or "",
            reader["enabled"], config,
        )
        return firmware_profile

    async def preview_esphome_config(self, request):
        self.require_admin_request(request)
        payload = await request.json()
        try:
            preview = await self.esphome_preview_data(payload)
    ...
    def remote_job_id(result):
        ...
        raise DeviceBuilderError("Device Builder did not return a firmware job ID")

    async def follow_device_builder_operation(self, operation_id):
        operation = self.registry.firmware_operation(operation_id)
        if not operation or not operation.get("remote_job_id"):
            if operation:
                self.registry.update_firmware_operation(
                    operation_id, status="failed", step="failed",
                    error="The remote Device Builder job ID is missing",
                    finished_at=now_iso(),
                )
            return
        ...
        async def record_event(event_name, data):
            ...
            if isinstance(data, dict):
                line = data.get("line") or data.get("message") or data.get("text")
                progress = data.get("progress")
                remote_status = str(data.get("status") or "").lower()
            else:
                line = data if name in {
                    "line", "log", "output", "stdout", "stderr"
                } else None
                progress = None
                remote_status = ""
            changes = {}
            if line:
                new_lines = [
                    item.rstrip() for item in str(line).splitlines() if item.strip()
                ]
                logs = (logs + new_lines)[-80:]
                changes["logs"] = logs
            if isinstance(progress, (int, float)):
                changes["progress"] = max(0, min(100, int(progress)))
            if remote_status in {"compiling", "installing", "uploading", "running"}:
                changes.update(status=remote_status, step=remote_status)
            if changes:
                self.registry.update_firmware_operation(operation_id, **changes)

        try:
            client = self.device_builder_client()
            following_tail = bool(operation.get("remote_tail_job_id"))
            current_job_id = (
                operation.get("remote_tail_job_id")
                or operation["remote_job_id"]
            )
            current_step = "installing" if following_tail else "compiling"
            self.registry.update_firmware_operation(
                operation_id, status=current_step, step=current_step, error=None
            )
            ...
            if isinstance(result, dict) and (
                not result.get("queued_update_armed")
                and (
                ...
                raise DeviceBuilderError(
                    ...
                    or "Device Builder firmware job failed",
                    "job_failed",
                )
            queued_update_armed = bool(
            ...
            if operation["install"] and not following_tail and direct_install_completed:
                ...
                self.registry.update_firmware_operation(
                    operation_id, logs=logs
                )
            elif operation["install"] and not following_tail:
                jobs = await client.command(
                    "firmware/get_jobs",
                    {"configuration": operation["configuration"]},
                )
                tail = next(
                    (
                        item for item in (jobs if isinstance(jobs, list) else [])
                        if isinstance(item, dict)
                        and str(item.get("depends_on") or "")
                        == operation["remote_job_id"]
                    ),
                    None,
                )
                if tail:
                    ...
                    tail_result = await client.command(
                        "firmware/follow_job",
                        {"job_id": tail_job_id},
                        on_event=record_event,
                        streaming=True,
                    )
                    if isinstance(tail_result, dict) and (
                        not tail_result.get("queued_update_armed")
                        and (
                            tail_result.get("success") is False
                            or str(tail_result.get("status", "")).lower() in {
                                "failed", "error", "cancelled"
                            }
                        )
                    ):
                        raise DeviceBuilderError(
                            tail_result.get("error") or tail_result.get("message")
                            or "Device Builder OTA upload failed",
                            "job_failed",
                        )
                else:
                    raise DeviceBuilderError(
            ...
            if error.code == "unavailable":
                self.registry.update_firmware_operation(
                    ...
                    error=f"Waiting for Device Builder: {str(error)[:420]}",
                )
    ...
    def start_firmware_follow(self, operation_id):
        task = asyncio.create_task(
            self.follow_device_builder_operation(operation_id)
        )
        self.firmware_tasks.add(task)
        task.add_done_callback(self.firmware_tasks.discard)
    ...
    async def build_esphome_firmware(self, request):
        ...
        try:
            if preview["exists"]:
                ...
                if configuration != f"{device_name}.yaml":
                    raise web.HTTPConflict(
                        text="New devices must use the configuration name generated from the device name"
                    )
                await client.command(
                    "devices/create",
                    {
                        "name": device_name,
                        "file_content": preview["yaml"],
                        "overwrite": False,
                    },
                )
            install = bool(payload.get("install"))
            remote_result = await client.command(
                "firmware/install" if install else "firmware/compile",
                {
                    "configuration": configuration,
                    **({"port": "OTA"} if install else {}),
                },
            )
        ...
        operation = self.registry.create_firmware_operation(
            operation_id, configuration, install,
            str(payload.get("reader_id") or "").strip() or None,
            remote_job_id,
        )
        reader_id = operation.get("reader_id")
        if reader_id:
            self.save_esphome_firmware_profile(payload, configuration)
        self.start_firmware_follow(operation_id)
    ...
    async def firmware_job(self, request):
        self.require_admin_request(request)
        job = self.registry.firmware_operation(str(request.match_info["job_id"]))
        if not job:
    ...
    def application(self):
        ...
        app.router.add_put(
            "/api/esphome/connection", self.save_esphome_connection
        )
        app.router.add_delete(
            "/api/esphome/connection", self.delete_esphome_connection
        )
        app.router.add_post(
        ...
        app.router.add_post(
            "/api/esphome/preview", self.preview_esphome_config
        )
