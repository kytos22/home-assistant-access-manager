...
ESPHome Fingerprint Access Reader 0.6.3 translates all stored finger positions using the display's live English/Spanish selection. Reader 0.6.2 explicitly publishes its diagnostic version entity during every boot, and reader 0.6.1 added per-device secret-key mapping for Access Manager generated wrappers. Reader 0.5.0 and Access Manager 0.10.0 introduced release-pinned configuration generation and reader version reporting. Both firmware profiles publish ESPHome project version metadata and a `Fingerprint reader firmware version` diagnostic sensor.
...
1. Generate the YAML in **Access Manager → Fingerprint readers → ESPHome reader configurator**, selecting the profile and **New device** or **Existing device update**.
